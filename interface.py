    ...
    if res_cnt > 0:
        ...
        if reservations_end_at_exclusive < end_block_inclusive + 1:
            ...
            for i in itertools.count():
                ...
                this_end_block = min(end_block_inclusive + 1,
                                     (i + 1) * BATCH_SIZE_BLOCKS + reservations_end_at_exclusive)
...
def get_reservation(curr: psycopg2.extensions.cursor, start_block: int, end_block_inclusive: int) -> typing.Optional[
    ReservationContextManager]:
    """
    ...
    if len(maybe_row) > 0:
        ...
        curr.execute('UPDATE gather_sample_arbitrages_reservations SET started_on = now()::timestamp WHERE id = %s',
                     (reservation_id,))
        assert curr.rowcount == 1
        curr.connection.commit()
...
def process_reservation(
    ...
    l.info(
        f'processing blocks {reservation_start:,} to {reservation_end_exclusive:,} ({reservation_end_exclusive - reservation_start:,} blocks)')
